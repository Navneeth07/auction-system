            ...
            {confirmationStep === 1 ? (
              <>
                ...
                <div className="flex gap-4">
                  <button onClick={() => setIsModalOpen(false)} className="flex-1 py-4 rounded-xl bg-white/5 border border-white/10 font-black uppercase text-xs">Back</button>
                  <button onClick={() => setConfirmationStep(2)} className="flex-[2] py-4 rounded-xl bg-amber-600 font-black uppercase text-xs hover:bg-amber-500 transition-all">Proceed</button>
                </div>
              </>
            ) : (
              <>
                ...
                <div className="flex gap-4">
                  <button onClick={() => setIsModalOpen(false)} className="flex-1 py-4 rounded-xl bg-white/5 border border-white/10 font-black uppercase text-xs opacity-50 hover:opacity-100">Abort</button>
                  <button onClick={executeHammerDown} className="flex-[2] py-4 rounded-xl bg-red-600 font-black uppercase text-xs shadow-2xl shadow-red-900/40 hover:bg-red-500 active:scale-95">Hammer Down 🔨</button>
                </div>
              </>
            )}
          </div>
      ...
      <div className="grid grid-cols-12 gap-4 flex-grow min-h-0 relative z-10">
        ...
        <div className="col-span-2 flex flex-col gap-4 min-h-0">
          <div className="bg-white/[0.03] backdrop-blur-xl p-4 rounded-3xl border border-white/10 shrink-0 shadow-lg">
            <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-3 italic opacity-60 italic">Pools</p>
        ...
        <div className="col-span-7 flex flex-col gap-4 min-h-0">
          ...
          {/* TEAM SELECTION - WITH PREMIUM HOVER EFFECTS */}
            ...
            {teams.map(team => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-8 h-[2px] mb-3 rounded-full transition-all duration-700 ease-out 
                    ${isActive ? "bg-amber-600 w-full" : "bg-white/10 group-hover:bg-amber-600 group-hover:w-full"}`} 
                  />

                  <div className={`border rounded-lg px-2.5 py-1.5 flex flex-col shadow-inner transition-colors 
                    ${isActive ? "bg-black/10 border-amber-600/40" : "bg-black/40 border-white/5 group-hover:bg-black/10 group-hover:border-amber-600/40"}`}
                  >
        ...
        {/* RIGHT PANEL - REDESIGNED SOLD LIST */}
