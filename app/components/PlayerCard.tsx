    ...
    return (
        <div className="flex items-center gap-4 p-4 rounded-xl bg-[#020617] border border-white/10">
            <div className="h-12 w-12 rounded-full bg-white/10 overflow-hidden">
                {player.image && (
                    <img src={player.image} alt={player.fullName} className="h-full w-full object-cover" />
                )}
            </div>

            <div className="flex-1">
                {/* Changed player.name to player.fullName to match your Player interface */}
                <p className="font-medium">{player.fullName}</p> 
                <span
                    className={`text-xs px-2 py-1 rounded ${roleColor[player.role] || "bg-gray-500/20 text-gray-400"}`}
                >
                    {player.role}
                </span>
            </div>
