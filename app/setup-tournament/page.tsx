...
export default function SetupTournamentPage() {
  ...
  return (
    <>
      ...
      <main className="h-screen overflow-y-auto custom-scroll bg-[#070d19] text-white px-6 py-12">
        {loading && <Loading />}
              ...
              {roles.map((r, i) => (
                <div key={i} className="grid grid-cols-4 gap-3 mb-4">
                  ...
                  <Input
                    label="Base Price"
                    value={r.basePrice}
                    onChange={(e) => updateRole(i, { basePrice: Number(e.target.value) })}
                    placeholder="₹2000"
                  />
                  <Input
                    label="Bid Price"
                    value={r.biddingPrice}
                    onChange={(e) => updateRole(i, { biddingPrice: Number(e.target.value) })}
                    placeholder="₹500"
                  />
                  <button
                    type="button"
...
function Input(props: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  ...
  return (
    <div className="mb-4">
      ...
      <input {...rest} className="w-full mt-1 bg-[#111b2e] px-4 py-2 rounded border border-gray-600" />
    </div>
  );
}
...
function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement> & { label: string }) {
  ...
  return (
    <div className="mb-4">
      <label className="text-sm text-gray-400">{label}</label>
      <textarea {...rest} className="w-full mt-1 bg-[#111b2e] px-4 py-2 rounded border border-gray-600" />
    </div>
  );
}